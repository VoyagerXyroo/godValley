...
(() => {
  ...
  var _ = (e) => {
    let t = (s, o, n) => {
      ...
      typeof f == "string"
        ? (a = f)
        : (a = f(n && "instanceIndex" in n ? n.instanceIndex : void 0));
  ...
  var K = ({ scriptAttributes: e, attributeKey: t, version: r, init: s }) => {
      var c;
      Z(), (c = window.fsAttributes)[t] || (c[t] = {});
      let { preventsLoad: o, attributes: n } = L(e),
        i = window.fsAttributes[t];
      (i.version = r),
        (i.init = s),
        o ||
          (window.Webflow || (window.Webflow = []),
          window.Webflow.push(() => s(n)));
    },
    Z = () => {
      let e = tt();
      ...
      let t = {
        ...
        push(...r) {
          var s, o;
          for (let [n, i] of r)
            (o = (s = this[n]) == null ? void 0 : s.loading) == null ||
              o.then(i);
        },
        destroy() {
          var r, s;
    ...
    tt = () => {
      let e = w("src", "finsweet", { operator: "contains" }),
        t = w("dev");
      return [...document.querySelectorAll(`script${e}, script${t}`)].reduce(
        (o, n) => {
          var c;
          let i =
            n.getAttribute(b.dev.key) ||
  ...
  var ot = `fs-${E}`,
    ...
    [O, x] = _(lt);
  var $ = () => {
    let e = x("previous", { operator: "prefixed" }),
      t = x("next", { operator: "prefixed" });
    if (!e && !t) return;
    let r = x("previousEmpty", { operator: "prefixed" });
    r == null || r.remove();
    let s = x("nextEmpty", { operator: "prefixed" });
    return (
      s == null || s.remove(),
      {
  ...
  var k = async () => {
    ...
    let t = !1,
      r = !1,
      s = e.createCMSListInstances([
        O("element", "list", { operator: "prefixed" }),
      ]),
      o = () =>
        B(E, s, () => {
          var u;
    ...
    let {
      ...
      { origin: f, pathname: T } = window.location,
      D = f + T,
      U = () => (!i || (i && t)) && (!c || (c && r));
    for (let u of s) {
      if (U()) break;
      let m = (R) => {
        if (U()) {
          u.off("additems", m), u.wrapper.remove();
          return;
        }
        ...
        if (!(S < 0)) {
          if (i) {
            let p = R[S - 1];
            p && !t
              ? (l == null || l.remove(), i.appendChild(p.element), (t = !0))
              : l && i.appendChild(l);
          }
          if (c) {
            let p = R[S + 1];
            p && !r
              ? (a == null || a.remove(), c.appendChild(p.element), (r = !0))
              : a && c.appendChild(a);
          }
        }
      };
      u.on("additems", m), m(u.items), (u.wrapper.style.display = "none");
    }
    return o();
  };
  K({ init: k, version: N, attributeKey: E });
})();
